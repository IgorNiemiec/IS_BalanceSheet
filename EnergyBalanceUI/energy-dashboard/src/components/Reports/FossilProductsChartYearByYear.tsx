...
export default function FossilYearComparisonChart() {
      ...
      for (const country of countries) {
        try {
          const res = await fetch(
            `http://localhost:5244/api/energy/report/by-product-filtered?country=${country.code}&year=${year}`
          );
  ...
  const getDisplayData = () => {
    ...
    const sumByProduct = (arr: CountryProductData[]) => {
      const map: Record<string, number> = {};
      arr.forEach(item => {
        map[item.productDescription] = (map[item.productDescription] || 0) + item.totalAmount;
      });
      return map;
    };

    const map1 = sumByProduct(filtered1);
    const map2 = sumByProduct(filtered2);
  ...
  const barColor1 = "#eb9800";
  const barColor2 = "#ff002b";

  return (
    <div className="energy-comparison-card">
      <div className="chart-controls">
        <h2>  Porównanie paliw kopalnych: rok do roku  </h2>
